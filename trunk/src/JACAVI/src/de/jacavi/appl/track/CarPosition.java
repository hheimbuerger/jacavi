...
public class CarPosition {
	...
	/**
	 * Whether the car is still on track. If this is false, all other values but
	 * {@link lap} have undefined values.
	 */
	...
	public CarPosition(Player p) {
		this.player = p;
		raceStatisticsManager = (RaceStatisticsManager) ContextLoader
				.getBean("statisticsRegistryBean");
	}
	...
	public void moveSteps(Track track, int stepsToMove,
			boolean laneChangeTriggered) {
		int remainingSteps = stepsToMove;

		while (remainingSteps > 0) {
			remainingSteps = moveOverTile(track, remainingSteps, track
					.getSections().get(trackSectionIndex).getTile());
		}
	}
	...
	private int moveOverTile(Track track, int remainingSteps, Tile currentTile) {
		...
		if (remainingSteps < commonSteps - stepsInTile) {
			...
			stepsInTile = 0;
			currentLane = isOnLaneChange ? lane.getChangeExitLaneIndex() : lane
					.getRegularExitLaneIndex();
			trackSectionIndex++;
