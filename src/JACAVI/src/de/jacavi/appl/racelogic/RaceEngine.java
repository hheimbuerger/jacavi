...
public class RaceEngine {
	...
	public void startRace(Track activeTrack, RaceView raceView) {
		...
		((RaceStatisticsManager) ContextLoader
				.getBean("statisticsRegistryBean")).init();
		...
		if (!isTimerRunning) {
			...
			TDAInjectorFactory tdaInjector = (TDAInjectorFactory) ContextLoader
					.getBean("tdaInjectorFactory");

			// TODO: assign all cars to their starting position
			int i = 0;
			for (Player player : players) {
				player.getPosition().reset(
						i++ % track.getTileset().getLaneCount());
			...
			logger
					.error("RaceEngine.startRace() was invoked but timer was already running. Race was *not* started!");
		}
	}
	...
	public void stopRace() {
		if (isTimerRunning) {
			...
			for (Player player : players) {
				...
				player.getSlotCarSystemConnector().getDriveConnector()
						.fullBreak();
			...
			logger
					.error("RaceEngine.stopRace() was invoked but timer was not running. Race was *not* stopped!");
		}
	}
	 ...
	 * Inner Class
	 * <p>
	 * Represents the RaceTimer polls the InputDevices and gives the signal to
	 * HAL and RaceView
	 * 
	 * @author fro
	 */
	class RaceTimerTask extends TimerTask {
		...
		public void run() {
			for (Player player : players) {
				...
				SlotCarSystemConnector slotCarSystemConnector = player
						.getSlotCarSystemConnector();
				// get players controller signal
				ControllerSignal controllerSignal = carController.poll();
				// get the hal feedback signal
				FeedbackSignal feedbackSignal = slotCarSystemConnector
						.pollFeedback();
				...
				tda.updatePosition(player.getPosition(), gametick, player
						.getCar(), track, controllerSignal, feedbackSignal);
